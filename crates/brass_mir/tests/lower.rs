...
fn first_fun(src: &str) -> FunDecl {
    let module = brass_parser::parse(src).expect("source parses");
    module
        .items
        .into_iter()
...
fn lower_first(src: &str) -> (MirBody, Vec<brass_mir::MirClosure>) {
...
fn lower_whole(src: &str) -> String {
    let ast = brass_parser::parse(src).expect("source parses");
