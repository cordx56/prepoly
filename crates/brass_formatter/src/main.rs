//! `czfmt`: the standalone formatter CLI. Prints one file's formatted text
...
fn main() -> ExitCode {
    ...
    if args.first().is_some_and(|a| a == "--version" || a == "-V") {
        println!("czfmt {}", brass_metadata::version_string());
    ...
    if args.is_empty() || (!write && args.len() != 1) {
        eprintln!("usage: czfmt FILE | czfmt --write|-w FILE... | czfmt --version");
    ...
    for path in &args {
        ...
        match brass_formatter::format_source(&src) {
            Ok(out) => {
                if write {
                    if out != src
            ...
            Err(errors) => {
                for e in errors {
                    let (line, col) = brass_parser::line_col(&src, e.span.lo);
