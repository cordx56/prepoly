...
impl<'a> Printer<'a> {
    ...
    fn int_text(&self, v: i64, span: Span) -> String {
        ...
        if let Ok(tokens) = prepoly_parser::lex(slice)
            && let [tok, _eof] = &tokens[..]
            && matches!(&tok.kind, prepoly_parser::TokenKind::Int(v2) if *v2 == v)
        {
    ...
    fn float_text(&self, v: f64, span: Span) -> String {
        ...
        if let Ok(tokens) = prepoly_parser::lex(slice)
            && let [tok, _eof] = &tokens[..]
            && matches!(&tok.kind, prepoly_parser::TokenKind::Float(v2) if v2.to_bits() == v.to_bits())
        {
            return slice.to_string();
        }
