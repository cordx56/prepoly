...
pub fn lower(modules: &[LoadedModule]) -> (Program, Vec<LowerError>) {
    ...
    let mut method_impls: Vec<(Rc<brass_parser::ast::FunDecl>, Vec<String>)> = Vec::new();
...
fn inject_method_impls(
    method_impls: &[(Rc<brass_parser::ast::FunDecl>, Vec<String>)],
    ...
    use brass_parser::ast::{FunDecl, Method, TypeExpr};
...
fn inject_nominal_method(
    info: &mut TypeInfo,
    method: brass_parser::ast::Method,
    errors: &mut Vec<LowerError>,
) {
    ...
    for m in members {
        match m {
            Member::Field(f) => {
                ...
                if matches!(f.ty, Some(brass_parser::ast::TypeExpr::TypeSlot(_))) {
...
mod tests {
    ...
    fn cross_module_same_function_name_coexists() {
        ...
        // imported into one module; see brass_resolve.)
        let modules: &[(&[&str], &str)] = &[
