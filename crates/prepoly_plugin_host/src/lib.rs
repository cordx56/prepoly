...
mod imp {
    ...
    fn staging_path(of: &Path) -> PathBuf {
        ...
        let ext = of
            ...
            .unwrap_or_default();
        std::env::temp_dir().join(format!("prepoly-plugin-{}-{n}{ext}", std::process::id()))
    }
    ...
    fn load_uncached(path: &Path, superseded: bool) -> Result<Loaded, String> {
            ...
            if raw.is_null() {
                return Err(format!(
                    "plugin `{}` rejected ABI v{ABI_VERSION} or failed to initialize; rebuild it \
                     against this prepoly_plugin version",
                    path.display()
                ));
            }
        ...
        let (call, release) = unsafe {
            let call: libloading::Symbol<CallFn> = lib.get(b"prepoly_call\0").map_err(|e| {
                format!(
                    "`{}` is not a Prepoly plugin (no `prepoly_call`): {e}",
                    path.display()
                )
            })?;
            let release = lib.get::<ReleaseFn>(b"prepoly_release\0").ok();
    ...
    /// `[A-Za-z_][A-Za-z0-9_]*`, the identifier shape both Rust and Prepoly
    /// accept.
    ...
    #[cfg(test)]
    mod tests {
        use prepoly_plugin::raw::{RawFunction, RawManifest, RawStr};
        ...
        fn decode(fns: &[RawFunction]) -> Result<Vec<String>, String> {
            let raw = RawManifest {
                abi: prepoly_plugin::raw::ABI_VERSION,
                fn_count: fns.len(),
                fns: fns.as_ptr(),
            };
        ...
        /// The manifest's names are spliced into synthesized Prepoly source and
...
pub mod fixture {
    ...
    /// `libraries/build.sh` produces and a Prepoly `import libprocess`
    ...
    pub fn build_faultylib() -> PathBuf {
        build_plugin("prepoly_plugin_faultylib", "prepoly_plugin_faultylib")
    }

    /// Build the fixture plugin that stands in for a rebuilt `build_testlib`.
    pub fn build_altlib() -> PathBuf {
        build_plugin("prepoly_plugin_altlib", "prepoly_plugin_altlib")
    }
}
