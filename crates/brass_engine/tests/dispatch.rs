...
use brass_engine::{Codegen, Engine, MonoFunction, MonoProgram, monomorphize};
use brass_hir::{LoadedModule, Type};
use brass_mir::{BlockId, LocalId, lower_program};
...
fn render(src: &str) -> String {
    ...
    let (program, _errors) = brass_hir::lower(&modules);
...
fn dispatch_monomorphizes_and_types_a_recursive_program() {
    ...
    let fact_i32 = brass_engine::instance_symbol("fact", &[Type::Int(brass_hir::IntKind::I32)]);
    assert!(
...
fn dispatch_specializes_one_function_for_two_types() {
    ...
    let id_i32 = brass_engine::instance_symbol("id", &[Type::Int(brass_hir::IntKind::I32)]);
    let id_f64 = brass_engine::instance_symbol("id", &[Type::Float(brass_hir::FloatKind::F64)]);
    assert!(
...
fn monomorphize_types_module_globals() {
    ...
    assert_eq!(
        mono.global_type(&brass_hir::qualify("counter", &["main".into()]))
            .map(|t| t.display()),
...
fn narrowed_nullable_array_ops_type_and_keep_the_init() {
    ...
    for (label, body) in [
        ...
        assert!(
            matches!(f.local_types.first(), Some(brass_hir::Type::Nullable(_))),
