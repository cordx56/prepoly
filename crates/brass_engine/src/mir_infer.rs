...
//! a type variable in the shared `brass_solver`, the body is walked emitting
...
use brass_solver::solver::{InferenceVarKind, Solver};
...
pub trait Resolver {
    ...
    fn call_type(&mut self, callee: &brass_mir::Callee, args: &[Type]) -> Option<Type>;
...
impl Resolver for NullResolver {
    fn call_type(&mut self, _: &brass_mir::Callee, _: &[Type]) -> Option<Type> {
        None
    }
...
impl Resolver for ProgramResolver<'_> {
    ...
    fn call_type(&mut self, callee: &brass_mir::Callee, _args: &[Type]) -> Option<Type> {
        match callee {
            brass_mir::Callee::Free(sym) => self
                .program
                .functions
                .get(sym)
...
pub fn infer_body<R: Resolver>(
    body: &MirBody,
    params: &[Type],
    captures: &[(brass_mir::LocalId, Type)],
    ret: Option<&Type>,
    fallible: bool,
    resolver: &mut R,
...
pub fn gather_requirements(body: &MirBody, deferred: brass_mir::LocalId) -> StructuralReq {
    let mut t = BodyTyper::new();
...
fn max_unknown_id(t: &Type, max: &mut u32) {
    match t {
        Type::Unknown(id) if *id != brass_hir::INFER_VAR => *max = (*max).max(*id),
        Type::Array(inner, _)
        | Type::Slice(inner)
        | Type::Nullable(inner)
        | Type::ConstOf(inner)
...
impl BodyTyper {
    ...
    /// conflict. Uses Brass's flow leniency (see [`BodyTyper::flow_unify`]) so a
    ...
    /// `brass_typesys::flow_unify`, shared with the HM checker so the two
    /// can never drift.
    ...
    fn rvalue_type<R: Resolver>(&mut self, rv: &Rvalue, resolver: &mut R) -> Type {
        match rv {
            ...
            Rvalue::Call(callee, args) => {
                ...
                if let brass_mir::Callee::Builtin(b) = callee
                    && b == "__deep_copy"
                ...
                if let brass_mir::Callee::Method(m) = callee
...
/// monomorphizer, and the back ends (re-exported as `brass_engine::is_comparison`).
...
mod tests {
    use super::*;
    use brass_mir::{BasicBlock, BlockId, LocalDecl, LocalId, TypeRef};
    ...
    fn infers_a_real_lowered_function() {
        ...
        let ast =
            brass_parser::parse("fun add(a: int32, b: int32) -> int32 {\n  return a + b\n}\n")
                .expect("parse");
