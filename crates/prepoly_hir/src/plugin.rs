//! The native-plugin value-type code, decoded into Prepoly types.
//!
...
//! string literal that builtin carries (see `prepoly_resolve::plugin`). Both
//! spellings are decoded here, once, so the checker, the back ends, and the
//! interpreter cannot drift from `prepoly_plugin::ValueType`, which owns the
//! encoding side. This module lives in `prepoly_hir` because the checker must
