//! LLVM IR generation. Each Brass function/method/closure becomes an LLVM
...
use brass_hir::{FloatKind, IntKind, NominalType, Program, Type, TypeKind};
...
/// A typed stack slot for one MIR local on the `brass_engine::Codegen` path:
...
/// `brass_engine::Codegen` trait to emit typed, fully unboxed code.
pub struct LlvmCodegen<'ctx, 'p> {
    ...
    /// State for the MIR-driven `brass_engine::Codegen` path.
    mir: MirState<'ctx>,
}

impl<'ctx, 'p> LlvmCodegen<'ctx, 'p> {
    /// Construct a code generator for the MIR-driven backend path. The resulting
    /// [`LlvmCodegen`] is handed to `brass_engine::Engine`, which drives it
    ...
    pub fn new_backend(ctx: &'ctx Context, program: &'p Program) -> Self {
        LlvmCodegen {
            ctx,
            module: ctx.create_module("brass"),
    ...
    fn run_optimization_passes(&self) {
        ...
        {
            tracing::warn!(target: "brass::ir", "optimization pipeline failed: {e}");
        }
    }
    ...
    for f in &program.functions {
        ...
        for block in &f.body.blocks {
            for stmt in &block.stmts {
                if let brass_mir::MirStmt::SetGlobal(name, _) = stmt {
...
impl<'ctx, 'p> LlvmCodegen<'ctx, 'p> {
    ...
    fn get_or_emit_deep_copy(&mut self, ty: &Type) -> FunctionValue<'ctx> {
        ...
        let result = match ty {
            ...
            Type::Slice(_) | Type::Array(..) => {
                let elem = brass_engine::element_type(ty);
                ...
                let copy_fn = if brass_engine::rc_managed(&elem) {
    ...
    fn get_or_emit_to_string(&mut self, ty: &Type) -> FunctionValue<'ctx> {
        ...
        let record = match ty {
            ...
            Type::Record(n) => {
                let header = if n.name == brass_hir::STRUCTURAL_RECORD_NAME {
                    "anonymous".to_string()
                } else {
                    n.name.clone()
                };
...
impl<'ctx, 'p> brass_engine::RuntimeJit for LlvmCodegen<'ctx, 'p> {
    ...
    /// cache/orchestration lives backend-agnostically in `brass_engine`.
    fn compile_instance(
        &mut self,
        program: &MonoProgram,
        f: &MonoFunction,
...
impl<'ctx, 'p> LlvmCodegen<'ctx, 'p> {
    ...
    pub fn emit_instance_module(
        ...
        let fresh = self.ctx.create_module("brass_rt");
...
impl<'ctx, 'p> EngineCodegen for LlvmCodegen<'ctx, 'p> {
    ...
    fn begin_program(&mut self, program: &MonoProgram) {
        ...
        self.region_barriers = brass_engine::program_uses_with(program);
    ...
    fn execute(&mut self) -> Result<(), String> {
        ...
        // (BRASS_LOG_TYPE=ir). Guarded so the module is only rendered when
    ...
    fn deep_copy(&mut self, value: BasicValueEnum<'ctx>, ty: &Type) -> BasicValueEnum<'ctx> {
        ...
        if brass_engine::rc_managed(&inner) && value.is_pointer_value() {
