//! Author native Prepoly plugins in Rust.
//!
//! A plugin is a `cdylib` crate. Implement [`PrepolyLib`] on a marker type,
//! define the functions with [`export!`] (which keeps the Rust doc comment,
//! shown by Prepoly's editor tooling), register them in `entry`, and expose
//! the library with [`prepoly_lib!`]:
//!
//! ```
//! use prepoly_plugin::{decl, export, prepoly_lib, PrepolyLib, Registry};
//!
//! export! {
//!     /// Adds two integers.
...
//! function body.)
//!
//! Build with `crate-type = ["cdylib"]` and import from Prepoly by the
...
/// A Prepoly plugin library: one type per `cdylib`, registering everything
/// the library exposes. Wire it up with [`prepoly_lib!`].
pub trait PrepolyLib {
    /// Register every function the library exposes.
    fn entry(reg: &mut Registry);
}

/// Define plugin functions, capturing each one's Rust doc comment and
/// parameter names for Prepoly's editor tooling. The functions stay ordinary
    ...
    ($( $(#[doc = $doc:literal])* $vis:vis fn $name:ident ( $($arg:ident : $aty:ty),* $(,)? ) $(-> $ret:ty)? $body:block )+) => {
        $(
            ...
            $vis mod $name {
                ...
                pub fn __prepoly_decl() -> $crate::FunctionDecl {
...
/// Expose a [`PrepolyLib`] implementation as this `cdylib`'s Prepoly entry
/// points (`prepoly_entry` / `prepoly_call` / `prepoly_release`; see
/// [`raw`]). Invoke exactly once per plugin crate.
#[macro_export]
macro_rules! prepoly_lib {
    ($lib:ty) => {
        #[unsafe(no_mangle)]
        pub extern "C" fn prepoly_entry(host_abi: u32) -> *const $crate::raw::RawManifest {
            $crate::__entry::<$lib>(host_abi)
        }

        #[unsafe(no_mangle)]
        pub unsafe extern "C" fn prepoly_call(
            index: u32,
        ...
        #[unsafe(no_mangle)]
        pub unsafe extern "C" fn prepoly_release(value: $crate::raw::RawValue) {
...
/// plugin on the first `prepoly_entry` call. The manifest address is stored
...
fn build_state<L: PrepolyLib>() -> State {
...
#[doc(hidden)]
pub fn __entry<L: PrepolyLib>(host_abi: u32) -> *const raw::RawManifest {
...
/// # Safety
/// `value` must come from this plugin's `prepoly_call` output and not have
...
mod tests {
    ...
    fn manifest() -> &'static raw::RawManifest {
        let m = prepoly_entry(raw::ABI_VERSION);
        assert!(!m.is_null());
        unsafe { &*m }
    }
    ...
    fn call(index: u32, args: &[raw::RawValue]) -> (i32, raw::RawValue) {
        ...
        let status = unsafe { prepoly_call(index, args.as_ptr(), args.len(), &mut out) };
        (status, out)
    }
    ...
    fn abi_mismatch_is_refused() {
        assert!(prepoly_entry(raw::ABI_VERSION + 1).is_null());
    }
}
